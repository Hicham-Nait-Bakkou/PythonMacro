...
def on_press(key):
    ...
    if hasattr(key, "char"):
        ...
        print(f'\nkey "{key}" pressed')
        if key.name == "cmd" and key.name != None:
...
def on_move(x, y):
    ...
    if not f"Pointer to {x},{y}" in keys:
...
class MainUI(QMainWindow):
    def __init__(self):
        ...
        exit_listener = keyboard.Listener(
            on_press=on_play_exit)
        exit_listener.start()
    ...
    def key_inputer(self):
        try:
            with open(macro_path, 'r') as macro:
                ...
                for line in lines:
                    keys.append(
                        line.removeprefix("Pointer to ").removeprefix("Scrolled ").removesuffix('\n')
                        )
...
class PopupWindow(QWidget):
    def __init__(self):
        ...
        self.hotkeyButton.clicked.connect(self.on_hotkey_clicked)
